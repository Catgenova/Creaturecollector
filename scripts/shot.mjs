...
await page.click('.sheet .close');
// the Battle Tower: stand just north of its door on the hub's south-west edge, step onto it, pick level 100, challenge the top floor
await page.evaluate(() => { const s = JSON.parse(localStorage.getItem('creaturecollector.save')); s.journey.player = { x: 108, y: 97, dir: 'down' }; localStorage.setItem('creaturecollector.save', JSON.stringify(s)); });
await page.reload();
await page.waitForSelector('canvas.ow-map');
await page.waitForTimeout(300);
await page.keyboard.press('ArrowDown');
await page.waitForSelector('.tower-row', { timeout: 5000 });
await page.click('.tower-tiers .btn:has-text("Lv 100")');
await page.screenshot({ path: path.join(out, 'world-tower.png'), fullPage: true });
console.log('tower:', JSON.stringify(await page.evaluate(() => ({ floors: document.querySelectorAll('.tower-row').length, tier: document.querySelector('.tower-tiers .btn.on') && document.querySelector('.tower-tiers .btn.on').textContent, meta: document.querySelector('.tower-row:last-child .tower-meta').textContent }))));
await page.click('.tower-row:last-child .btn');
await page.waitForSelector('.encounter.tower', { timeout: 5000 });
await page.screenshot({ path: path.join(out, 'world-tower-encounter.png'), fullPage: true });
